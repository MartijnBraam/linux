					 ...
					 union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		val->intval = (bool)(tmp & PM8916_LBC_USB_USBIN_VALID);
...
static irqreturn_t pm8916_lbc_charger_state_changed_irq(int irq, void *data)
{
	...
	regmap_read(chg->regmap, chg->reg[LBC_USB] + PM8916_INT_RT_STS, &tmp);
	extcon_set_state_sync(chg->edev, EXTCON_USB, (bool)(tmp & PM8916_LBC_USB_USBIN_VALID));
...
static int pm8916_lbc_charger_probe_dt(struct pm8916_lbc_charger *chg)
{
	...
	if (tmp == 0) {
		ret = regmap_write(chg->regmap,
				   chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_TCHG_MAX_EN, 0x00);
	} else {
		ret = regmap_write(chg->regmap, chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_TCHG_MAX_EN,
				   PM8916_LBC_CHGR_TCHG_MAX_ENABLED);

		tmp = min_t(u32, max_t(u32, PM8916_LBC_CHGR_MIN_TIME, tmp),
			    PM8916_LBC_CHGR_MAX_TIME) / PM8916_LBC_CHGR_MIN_TIME - 1;
		ret = regmap_write(chg->regmap,
				   chg->reg[LBC_CHGR] + PM8916_LBC_CHGR_TCHG_MAX, tmp);
	}
