...
#define PM8916_SEC_MAGIC 0xA5	// FIXME Not sure what this magic 0xA5 means
					      ...
					      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		// NOTE: Maybe not the best way but I just assume that
		// if at leastone psu is online then it is charging
...
static irqreturn_t pm8916_bms_vm_fifo_update_done_irq(int irq, void *data)
{
	...
	struct power_supply_battery_info *info = &bat->info;
	int i, delta = 0, loc_delta;
	unsigned int tmp = 0;
	int supplied;

	// FIXME Huge magic all over the place
...
static int pm8916_bms_vm_battery_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	//FIXME: Should I check this pm event?
